...
def train(epoch, X_train):
    ...
    for i in range(0, X_train.size(0), batch_size):
        ...
        optimizer.zero_grad()
        output = model(x)
        loss = model.loss_function(*output, M_N = 0.0045*(bs/num_train_img))
        #print(loss)
        loss['loss'].backward()
        if clip > 0:
...
def evaluate(epoch, X_test):
    ...
    with torch.no_grad():
        ...
        test_loss = model.loss_function(*output, M_N = 0.0045)
...
if __name__ == "__main__":
    ...
    #input_channels, num_filters, latent_dim, hidden_dims, kernels_size)
    ...
    train_loss = []
    test_loss = []
    model_name = "ConvVAE_KL0045rel_bs"+str(batch_size)+"_do"+str(int(dropout*100))+"_seq"+str(seq_len)+"_"+val_mask_name+"_phase"+str(phase)+"_nlf"+str(latent_dim)
